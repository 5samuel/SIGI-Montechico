...
function StockActualPorProducto(){
    ...
    const {reportStockProductosTodos,buscarproductos,buscardor,setBuscador,selectproductos}= useProductosStore();
    ...
    const {data:dataproductosbuscador, isLoading:ProductosBuscador, error:errorBuscador} = useQuery({
        queryKey:["buscar productos", {id_empresa:dataempresa?.id, descripcion:buscardor}],
        queryFn:()=>buscarproductos({id_empresa:dataempresa?.id, descripcion:buscardor}),
        enabled: !! dataempresa,
    })
    ...
    section:{margin:20, padding:10, flexGrow:1},
